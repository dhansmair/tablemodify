...
class CellFactory {
    ...
    create(colSettings, i) {
        ...
        if (!colSettings.hasOwnProperty('options')) {
            colSettings.options = null
        }
...
class Controller {
    constructor(settings) {
        ...
        // replace tm-ids with actual indices
        Object.keys(this.settings.columns).forEach((key) => {
            if (key != 'all' && isNaN(key)) {
                let id = tm.id2index(key)
                if (id != null) {
                    this.settings.columns[id] = this.settings.columns[key]
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (bundle.handler && bundle.handler.hasPanel()) {
